...
func (c *Crawler) do(pkgPath string) (*domain.Package, error) {
	...
	targetPath := filepath.Join(os.TempDir(), "src")
	localPath := filepath.Join(targetPath, rr.Root)
	...
	if err := c.get(targetPath, rr); err != nil {
		finishedAt := time.Now()
	...
	pc, err := c.interrogate(targetPath, rr)
...
func (c *Crawler) get(targetPath string, rr *vcs.RepoRoot) error {
	dst := filepath.Join(targetPath, rr.Root)
	if err := os.RemoveAll(dst); err != nil {
		return err
	}
	if err := os.MkdirAll(targetPath, os.FileMode(int(0755))); err != nil {
		return err
	}
...
func (c *Crawler) interrogate(targetPath string, rr *vcs.RepoRoot) (*domain.PackageCrawl, error) {
	localPath := filepath.Join(targetPath, rr.Root)
	...
	scanDir := func(dir string) {
		pkgPath := strings.Replace(dir, targetPath+string(os.PathSeparator), "", 1)
		goPkg, err := loadPackageDynamic(targetPath, pkgPath)
		if err != nil {
